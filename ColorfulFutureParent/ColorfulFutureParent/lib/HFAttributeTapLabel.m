...
@implementation HFAttributeModel

@end


@interface HFAttributeTapLabel()

@property (nonatomic, strong) NSArray *targetStringArray;

@end

@implementation HFAttributeTapLabel

- (void)setText:(NSString *)text
     attributes:(NSDictionary *)attr
 tapStringArray:(NSArray <HFAttributeModel *>*)stringArray
{
    ...
    [stringArray enumerateObjectsUsingBlock:^(HFAttributeModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    for (CFIndex i = 0; i < numberOfLines; i++) {
        ...
        if (CGRectContainsPoint(rect, p)) {
            ...
            for (int j = 0; j < link_count; j++) {
                HFAttributeModel *model = self.targetStringArray[j];
