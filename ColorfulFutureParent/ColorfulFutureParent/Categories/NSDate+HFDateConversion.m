...
@implementation NSDate (HFDateConversion)
...
+ (NSString *)currentStringWithFormat:(NSString *)format {
    return [[NSDate date] currentStringWithFormat:format];
}
...
+ (NSString *)stringWithDate:(NSDate *)date format:(NSString *)format {
    return [date currentStringWithFormat:format];
}
...
//    return [formatter stringFromDate:timeDate];
    ...
    NSString *creat = [NSString stringWithFormat:@"%ld",timestamp];
    NSString *sub = [creat substringToIndex:10];
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateStyle:NSDateFormatterMediumStyle];
    [formatter setTimeStyle:NSDateFormatterShortStyle];
    [formatter setDateFormat:format];
    NSDate*confromTimesp = [NSDate dateWithTimeIntervalSince1970:(NSTimeInterval)[sub intValue]];
